...
public class JobScheduler implements ServletContextListener {
    ...
    public void run() throws SchedulerException {
        ...
        //NurseReminder daily
//        addJob("NurseReminder", GROUPS.MAILING, NurseReminderJob.class, CronScheduleBuilder.cronSchedule("0 0 0 1/1 * ? *"));
        
        if(!CMS_Config_Std.getInstanceXmlOnly().SchedulerEnabled) {
            log.info("Scheduler is disabled by config file, ignoring Jobs");
        }else {
          //addJob("NurseReminder", GROUPS.MAILING, NurseReminderJob.class, CronScheduleBuilder.cronSchedule("0 0 7 ? * MON,WED,FRI *"));
