...
import archenoah.web.normal.main.Menuepunkte.Individuelle.Stammdaten.Patientenverwaltung.Form.Filter.Decorator_Arzt_Zuweisung_Patient;
import archenoah.web.normal.main.Menuepunkte.Individuelle.Stammdaten.Patientenverwaltung.Form.Filter.Generator_Arzt_Zuweisung_Patient;
...
public class data_view_behandelnderarzt extends CustomComponent {
    ...
    /************ ----->Einstellungen<-- ********************/
//    private String LNG_FRM_ID = "16";
    private String BER_MEN_ID = "26";
    private String Windows_Height = "500px";
    private String Windows_Width = "800px";
    private int Datenholen_MS = 5000;
    private boolean Datenholen_An = true;

    private String TBL_ID = "VAP_ID";

    private String NOT_Del = "27";
    ...
    public data_view_behandelnderarzt(MenuItem Arg_Menü) {
        Temp_Menü = Arg_Menü;

        // TODO add user code here
    }

    public data_view_behandelnderarzt(Button Arg_btn) {
    ...
    public void Set_Berrechtigung_Custom(int Arg_Lesen, int Arg_Erstellen, int Arg_Bearbeiten, int Arg_Loeschen) {
        Lesen = Arg_Lesen; // read
        Schreiben = Arg_Erstellen; // update
        Erstellen = Arg_Erstellen; // create
        Edit = Arg_Bearbeiten; // update
        Löschen = Arg_Loeschen; // delete
    }

    public void Set_Form_for_Ber() {

        if (Lesen == 0) {
            Btn_INFO.setEnabled(false);
        }
        if (Schreiben == 0 && Edit == 0) {
    ...
    public void Set_Patienten_ID(String Arg_ID) {
        if (Arg_ID == "") {
            P_ID = "0";
        } else {
            P_ID = Arg_ID;
    ...
    public ClickCommand cmd_btn_add = new ClickCommand() {
        ...
        public void menuClicked(MenuItem selectedItem) {
            ...
            form_zuweisung_pat_arzt pa = new form_zuweisung_pat_arzt(selectedItem, "");
            pa.PatientenID = P_ID;
            pa.Init_Class_Window();
            this.setParent(men_frm);
            this.setWindow(pa.getWindow());
    ...
    public ClickCommand cmd_btn_edit = new ClickCommand() {
        ...
        public void menuClicked(MenuItem selectedItem) {
            ...
            form_zuweisung_pat_arzt pa = new form_zuweisung_pat_arzt(selectedItem, ID + "");
            pa.PatientenID = P_ID;
            pa.Init_Class_Window();
            this.setParent(men_frm);
            this.setWindow(pa.getWindow());
    ...
    public ClickCommand cmd_btn_info = new ClickCommand() {
        @Override
        public void menuClicked(MenuItem selectedItem) {
            // TODO Auto-generated method stub
            ...
            final int ID = (int) tbl_frm.getContainerProperty(rowId, "VAP_A_ID").getValue();

            form_arzt_insert_edit pie = new form_arzt_insert_edit(selectedItem, "" + ID);
    ...
    private void Generate_Menüs() {
        ...
        Btn_INFO = men_frm.addItem("FRM_INFO", Icons.White.info_16, cmd_btn_info);
    ...
    private Container Get_Data_Container() {
        try {
            ...
            /************ Set Spalten *************/
            db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("VAP_ID");
            db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("VAP_A_ID");
            db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("VAP_B_ID");
            db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("CONCAT_WS(' ', ASA_TITEL,KSA_NAME,AUL_NAME,AUL_VORNAME)", "Arztname");
            db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("AAS_NAME", "Art");
            db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("VAP_BEMERKUNGEN", "Bemerkungen");

            /**************************************/
            /************ Set Tabelle *************/
            db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Verknüpfung("cust_verk_arzt_patient", "INNER JOIN", "cust_azt_art_stammdaten", "VAP_B_ID", "AAS_ID");
            ...
            /**************************************/
            /************ Set Filter *************/
            db.DB_Data_Get.DB_Filter.DB_WHERE_Allgemein("cust_verk_arzt_patient", "VAP_P_ID", "=", P_ID + "", "");
    ...
    private void Datenholen() {
        Container con = Get_Data_Container();
        // System.out.println("datenholen "+con.size());
        if (con.size() > 0) {
            ...
            tbl_frm.setColumnFooter("Bemerkungen", "Insgesamt : " + tbl_frm.size() + " Ärzte");

            tbl_frm.setVisibleColumns(new Object[] { "Arztname", "Art", "Bemerkungen" });
    ...
    private boolean Prüfengleichheit(Container con) {
        ...
        if (con.size() != cont_Momentan.size()) {
            gl = false;
        } else {
    ...
    private void Del(int ID) {
        ...
        DBClass db = new DBClass();
        db.DB_Data_Delete.DB_Tabellen.DB_set_Tabelle_Einzeln("cust_verk_arzt_patient");
        db.DB_Data_Delete.DB_Filter.DB_WHERE_Allgemein("cust_verk_arzt_patient", "VAP_ID", "=", ID + "", "");
    ...
    private void Config_TBL() {
        ...
        tbl_frm.setFilterDecorator(new Decorator_Arzt_Zuweisung_Patient());
        tbl_frm.setFilterGenerator(new Generator_Arzt_Zuweisung_Patient());

        tbl_frm.setColumnReorderingAllowed(true);
        tbl_frm.setContainerDataSource(Get_Data_Container());
        tbl_frm.getFilterDecorator().getEnumFilterDisplayName("Art", "Hausarzt");
        // tbl_arzt.setFilterBarVisible(true);
        tbl_frm.setFooterVisible(true);
        tbl_frm.requestRepaint();
