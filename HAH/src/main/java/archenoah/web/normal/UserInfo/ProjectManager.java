...
public class ProjectManager {
 ...
 // {section fields}
    ...
    private HashMap<String, ArrayList<Integer>> idsByCodeList;
    
    // {end fields}

    // {section constructors}
    // **********************
    public ProjectManager(Integer userId) {
        this.userId = userId;
        getPermissions();
    }
    // {end constructors}
    ...
    public ArrayList<Integer> getProjectIdsForCode(String projectCode) {
        return new ArrayList<Integer>(idsByCodeList.get(projectCode));
    }
    ...
    private void getPermissions() {
        ...
        for (Object iid : projects.getItemIds()) {
            ...
            if(idsByCodeList.get(MyUtils.getValueFromItem(item, "projectCode", String.class)) == null) {
                idsByCodeList.put(MyUtils.getValueFromItem(item, "projectCode", String.class), new ArrayList<Integer>());
            }
            idsByCodeList.get(MyUtils.getValueFromItem(item, "projectCode", String.class))
                .add(MyUtils.getValueFromItem(item, "projectId", Integer.class));
            
        }
        
        isProjectManager = projects.size() > 0;
    ...
    private Container dbGetProjects() {
        ...
        String sql = "select"
         ...
         + "\n " + "inner join cust_projekte_stammdaten_liste on SPL_ID_PROJEKT = PSLV_ID"
         + "\n " + "where SPL_ID_USER = " + userId;
        q.setSqlString(sql);
        return q.query();
    }
    // {end database}
