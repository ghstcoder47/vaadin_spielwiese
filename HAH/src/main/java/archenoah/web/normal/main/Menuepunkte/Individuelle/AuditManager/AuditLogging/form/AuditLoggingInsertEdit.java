package archenoah.web.normal.main.Menuepunkte.Individuelle.AuditManager.AuditLogging.form;
...
import archenoah.web.normal.main.Menuepunkte.Individuelle.AuditManager.AuditLogging.classes.AuditFindingContainer;
import archenoah.web.normal.main.Menuepunkte.Individuelle.AuditManager.AuditLogging.classes.AuditFindingContainer.OnRatingChange;
import archenoah.web.normal.main.Menuepunkte.Individuelle.AuditManager.AuditLogging.classes.AuditFindingContainer.OnSave;
...
public class AuditLoggingInsertEdit extends CustomComponent {
    ...
    @AutoGenerated
    private VerticalLayout layoutFindings;

    @AutoGenerated
    private Panel panelTopics;

    @AutoGenerated
    private VerticalLayout layoutFindingsPanel;

    @AutoGenerated
    private Accordion wrapperFindings;
    ...
    @AutoGenerated
    private TextField fieldNewFindingTitle;
    ...
    @AutoGenerated
    private TextField fieldAuditor;

    @AutoGenerated
    private ComboBox fieldDivision;
    ...
    /************ ----->Einstellungen<-- ********************/
//    private final String formIdLng = "502";
    private final String formIdPerm = "501";
    private final String Windows_Height = "600px";
    private final String Windows_Width = "800px";
    
    private final String dataIdCol = "ALL_ID";
    private final String dataTableName = "cust_audit_logging_logs";
    
    /******************************************************/

    public AuditLoggingInsertEdit(MenuItem Arg_Menü, String Arg_Form_ID) {
        tmpMenu = Arg_Menü;
        dataSetId = Arg_Form_ID;
    }

    public AuditLoggingInsertEdit(Button Arg_btn, String Arg_Form_ID) {
        tmpButton = Arg_btn;
        dataSetId = Arg_Form_ID;
    }
    ...
    private void Standard_Init() {
        ...
        wrapperFindings.addTab(new VerticalLayout(), "Alle Findings zusammenklappen");
...
//    	for (int i = 1; i < 2; i++) {
//    	    
//    	    wrapperTopics.addTab(new AuditFindingContainer(), "TOP " + i);
//            
//        }
    	
    	wrapperFindings.setHeight(null);
    	layoutFindingsPanel.setHeight(null);
    	
    	
//    	fieldAddUser.setItemCaptionPropertyId("user");
    	fieldAddUser.setNullSelectionAllowed(true);
    	fieldType.setItemCaptionPropertyId("ALT_NAME");
    	fieldType.setNullSelectionAllowed(false);
    	
    	fieldDivision.setItemCaptionPropertyId("ALD_NAME");
    	fieldDivision.setNullSelectionAllowed(false);
    	
    	listUsers.setNullSelectionAllowed(false);
    	
    	fieldNewFindingTitle.setNullRepresentation("");
    	
    	componentListeners();
    	
    	ValueChangeListener titleListener = new ValueChangeListener() {
        ...
        fieldDate.addValueChangeListener(titleListener);
        fieldType.addValueChangeListener(titleListener);
        fieldDivision.addValueChangeListener(titleListener);
    ...
    private void setWindowTitle() {
        if(win != null) {
            ...
            String date = fmt.format(fieldDate.getValue());
            String type = fieldType.getValue() != null ? fieldType.getItemCaption(fieldType.getValue()) : null;
            String division = fieldDivision.getValue() != null ? fieldDivision.getItemCaption(fieldDivision.getValue()) : null;
            ...
            if(taskManagerDataView != null) {
                taskManagerDataView.setTitle(win.getCaption());
            }
        }
    }
    ...
    private void componentListeners(){
    	...
    	wrapperFindings.addSelectedTabChangeListener(new SelectedTabChangeListener() {
            
            @Override
            public void selectedTabChange(SelectedTabChangeEvent event) {
                if(event.getTabSheet().getSelectedTab() instanceof AuditFindingContainer) {
                    AuditFindingContainer mtc = (AuditFindingContainer) event.getTabSheet().getSelectedTab();
        //                mtc.presetTitle(win.getCaption());
                    mtc.updateTaskList();
    ...
    private void newFinding(){
        ...
        if(fieldNewFindingTitle.getValue() != null && !fieldNewFindingTitle.getValue().equals("")){
            
            newFinding(fieldNewFindingTitle.getValue());
            fieldNewFindingTitle.setValue(null);
            
        }
        
    }
    
    private OnSave getOnSaveAction() {
        return new OnSave() {
            @Override
    ...
    private OnRatingChange getOnRatingChangeAction() {
        return new OnRatingChange() {
            
            @Override
            public void onRatingChangeAction(AuditFindingContainer mtc, String rating) {
                wrapperFindings.getTab(mtc).setCaption(parseTopId(mtc) + "[" + rating + "] " + mtc.getTopTitle());
            }
        };
    }
    
    private String parseTopId(AuditFindingContainer mtc) {
        return mtc.getTopId() != null ? "#" + mtc.getTopId() + " " : "";
    }
    
    private void newFinding(String findingName){
        
        Integer logId = dataSetId.equals("") ? null : Integer.parseInt(dataSetId);
        
        AuditFindingContainer mtc = new AuditFindingContainer(taskManagerDataView, ratingConverter, logId, findingName);
        mtc.setButtonsEnabled(btn_save.isEnabled());
//        mtc.presetTitle(win.getCaption());
        
        Tab tab = wrapperFindings.addTab(mtc, parseTopId(mtc) + mtc.getTopTitle());
        mtc.setTab(wrapperFindings, tab);
        mtc.setSaveAction(getOnSaveAction());
        mtc.setRatingChangeAction(getOnRatingChangeAction());
        
        wrapperFindings.setSelectedTab(tab);
    ...
    private void getTopics(){
        ContainerTool ct = new ContainerTool(dbGetFindings());
        ct.iterateItems(ct.new IterationInstruction() {
            ...
            public void runInstruction(ContainerTool cti, Object itemId, Item item, Object[] objects) {
                ...
                AuditFindingContainer mtc = new AuditFindingContainer(taskManagerDataView, ratingConverter, (Integer) item.getItemProperty("ALF_ID").getValue());
                mtc.setButtonsEnabled(btn_save.isEnabled());
                Tab tab = wrapperFindings.addTab(mtc, parseTopId(mtc) + mtc.getTopTitle());
                mtc.setTab(wrapperFindings, tab);
                mtc.setSaveAction(getOnSaveAction());
    ...
    private void initTaskManager(){
        ...
        if(taskManagerDataView == null){
            ...
            taskManagerDataView.setType(TTYPE.CAPA);
    ...
    private Integer saveForm(){
        ...
        for (Iterator iterator = wrapperFindings.iterator(); iterator.hasNext();) {
            try {
                Object finding = iterator.next();
                if(finding instanceof AuditFindingContainer) {
                    AuditFindingContainer mtc = (AuditFindingContainer) finding;
    ...
    private void setMenuPermissions() {
        ...
        buttonNewTopicAdd.setEnabled(c||u);
        buttonAddUser.setEnabled(c||u);
        buttonRemoveUsers.setEnabled(c||u);
    ...
    private void fillFields(){
        ...
        dbGetType();
        dbGetDivision();
        ratingConverter = new I18nConverter("cust_audit_master_rating", "AMR_ID", "AMR_TOKEN");
...
//        dbMap.put(component, "table:COLUMN");
        ...
        dbMap.put(fieldDate,  dataTableName + ":ALL_DATE");
        dbMap.put(fieldTimeStart,  dataTableName + ":ALL_TIME_START");
        dbMap.put(fieldTimeEnd,  dataTableName + ":ALL_TIME_END");
        
        dbMap.put(fieldType,  dataTableName + ":ALL_ID_TYPE");
        dbMap.put(fieldDivision,  dataTableName + ":ALL_ID_DIVISION");
        dbMap.put(fieldAuditor,  dataTableName + ":ALL_AUDITOR");
        
        dbMap.put(fieldGuests,  dataTableName + ":ALL_GUESTS");
    ...
    private void dbGetUsers(){
        ...
        String tbl = "cust_audit_logging_participants";
        ...
        db.DB_Data_Get.DB_Filter.DB_WHERE_Allgemein(tbl, "ALP_ID_AUDIT", "=", dataSetId, "");
        ...
        if(con.size() > 0){
            ...
            for (Object cid : con.getItemIds()) {
                ...
                Integer uid = (Integer) con.getContainerProperty(cid, "ALP_ID_USER").getValue();
    ...
    private void dbGetType() {
        DBClass db = new DBClass();

        db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("ALT_ID");
        db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("ALT_NAME");
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Einzeln("cust_audit_logging_type");
        
//        db.debugNextQuery(true);
        
        fieldType.setContainerDataSource(MyUtils.convertIndex(db.DB_Data_Get.DB_SEND_AND_GET(), "ALT_ID"));
    }
    
    private void dbGetDivision() {
        DBClass db = new DBClass();

        db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("ALD_ID");
        db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_Einzeln("ALD_NAME");
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Einzeln("cust_audit_logging_division");
        
//        db.debugNextQuery(true);
        
        fieldDivision.setContainerDataSource(MyUtils.convertIndex(db.DB_Data_Get.DB_SEND_AND_GET(), "ALD_ID"));
...
//        TMEP_ID_USER
        ...
        db.DB_Data_Delete.DB_Filter.DB_WHERE_Allgemein(tbl, "ALP_ID_AUDIT", "=", dataSetId, "");
        ...
        for (Object user : (Collection) listUsers.getItemIds()) {
            ...
            db.DB_Data_Insert.DB_Daten.DB_Data(tbl, "ALP_ID_AUDIT", dataSetId);
            db.DB_Data_Insert.DB_Daten.DB_Data(tbl, "ALP_ID_USER", user.toString());
    ...
    private void dbTouch() {
        ...
        db.DB_Data_Update.DB_Daten.DB_Data(dataTableName, "ALL_UPDATED", MyUtils.formatSqlDate(new Date()));
    ...
    private Container dbGetFindings(){
        
        DBClass db = new DBClass();
        db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_All();
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Einzeln("cust_audit_logging_findings");
        db.DB_Data_Get.DB_Filter.DB_WHERE_Allgemein("cust_audit_logging_findings", "ALF_ID_AUDIT", "=", dataSetId, "");
...
//        ValidateMap.put(component, "component"); // should not be needed
    ...
    private HorizontalLayout buildLayoutHeader() {
        ...
        fieldType.setCaption("Auditart");
        fieldType.setImmediate(false);
        fieldType.setWidth("100.0%");
        fieldType.setHeight("-1px");
        layoutHeader.addComponent(fieldType);
        layoutHeader.setExpandRatio(fieldType, 1.0f);
        
        // fieldDivision
        fieldDivision = new ComboBox();
        fieldDivision.setCaption("Auditierter Bereich");
        fieldDivision.setImmediate(false);
        fieldDivision.setWidth("100.0%");
        fieldDivision.setHeight("-1px");
        layoutHeader.addComponent(fieldDivision);
        layoutHeader.setExpandRatio(fieldDivision, 1.0f);
        
        // fieldAuditor
        fieldAuditor = new TextField();
        fieldAuditor.setCaption("Auditor");
        fieldAuditor.setImmediate(false);
        fieldAuditor.setWidth("100.0%");
        fieldAuditor.setHeight("-1px");
        layoutHeader.addComponent(fieldAuditor);
        layoutHeader.setExpandRatio(fieldAuditor, 1.0f);
    ...
    private TabSheet buildTabMain() {
        ...
        // layoutFindings
        layoutFindings = buildLayoutFindings();
        tabMain.addTab(layoutFindings, "Findings", null);
    ...
    @AutoGenerated
    private VerticalLayout buildLayoutFindings() {
        // common part: create layout
        layoutFindings = new VerticalLayout();
        layoutFindings.setImmediate(false);
        layoutFindings.setWidth("100.0%");
        layoutFindings.setHeight("100.0%");
        layoutFindings.setMargin(false);
        layoutFindings.setSpacing(true);
        
        // layoutNewTopic
        layoutNewTopic = buildLayoutNewTopic();
        layoutFindings.addComponent(layoutNewTopic);
        
        // panelTopics
        panelTopics = buildPanelTopics();
        layoutFindings.addComponent(panelTopics);
        layoutFindings.setExpandRatio(panelTopics, 1.0f);
    ...
    private HorizontalLayout buildLayoutNewTopic() {
        ...
        // fieldNewFindingTitle
        fieldNewFindingTitle = new TextField();
        fieldNewFindingTitle.setCaption("Neues Finding hinzufügen");
        fieldNewFindingTitle.setImmediate(false);
        fieldNewFindingTitle.setWidth("100.0%");
        fieldNewFindingTitle.setHeight("-1px");
        layoutNewTopic.addComponent(fieldNewFindingTitle);
        layoutNewTopic.setExpandRatio(fieldNewFindingTitle, 1.0f);
    ...
    private Panel buildPanelTopics() {
        ...
        // layoutTopicsPanel
        layoutFindingsPanel = buildLayoutTopicsPanel();
        panelTopics.setContent(layoutFindingsPanel);
    ...
    private VerticalLayout buildLayoutTopicsPanel() {
        // common part: create layout
        layoutFindingsPanel = new VerticalLayout();
        layoutFindingsPanel.setImmediate(false);
        layoutFindingsPanel.setWidth("100.0%");
        layoutFindingsPanel.setHeight("100.0%");
        layoutFindingsPanel.setMargin(false);
        
        // wrapperTopics
        wrapperFindings = new Accordion();
        wrapperFindings.setImmediate(false);
        wrapperFindings.setWidth("100.0%");
        wrapperFindings.setHeight("100.0%");
        layoutFindingsPanel.addComponent(wrapperFindings);
        layoutFindingsPanel.setExpandRatio(wrapperFindings, 1.0f);
        
        return layoutFindingsPanel;
    }
