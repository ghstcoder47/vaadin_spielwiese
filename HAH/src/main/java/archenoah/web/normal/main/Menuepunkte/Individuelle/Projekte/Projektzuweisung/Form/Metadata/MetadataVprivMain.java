...
public class MetadataVprivMain extends MetadataPanel {
//    public class MetadataVprivMain extends CustomComponent {
    ...
    // 	Datenbank
    private final String projectId = "3";
    
    final String table_static = "cust_protokoll_vpriv_meta_static";
    final String table_dynamic = "cust_protokoll_vpriv_meta_dynamic";
...
//    /*************************************/
    ...
    private void editDynamicFields(Object itemId) {
        ...
        if (item != null) {
            ...
            metaDynamicId = (Integer) item.getItemProperty("PVMD_ID").getValue();

            pop_dyn_dateDelegation.setValue((Date) item.getItemProperty("PVMD_DELEGATION").getValue());
            pop_dyn_dateActive.setValue((Date) item.getItemProperty("PVMD_ACTIVE").getValue());
            int_volume.setValue((Integer) item.getItemProperty("PVMD_VOLUME").getValue());
            cb_homecare.setValue((Boolean) item.getItemProperty("PVMD_HOMECARE").getValue());
            txt_rrule_id.setValue(Integer.toString((Integer) item.getItemProperty("PVMD_RRULE_ID").getValue()));
            metaDynamicHomecareValue = (Boolean) item.getItemProperty("PVMD_HOMECARE").getValue();
    ...
    private void setDBMaps() {
        ...
        final String table_static = this.table_static + ":";
        final String table_dynamic = this.table_dynamic + ":";
        
        DBMapDynamic = new HashMap<Object, String>();
        DBMapDynamic.put(pop_dyn_dateDelegation, table_dynamic + "PVMD_DELEGATION");
        DBMapDynamic.put(pop_dyn_dateActive, table_dynamic + "PVMD_ACTIVE");
        DBMapDynamic.put(int_volume, table_dynamic + "PVMD_VOLUME");
        DBMapDynamic.put(cb_homecare, table_dynamic + "PVMD_HOMECARE");
        DBMapDynamic.put(txt_rrule_id, table_dynamic + "PVMD_RRULE_ID");

        DBMapStatic = new HashMap<Object, String>();
        DBMapStatic.put(txt_patCode, table_static + "PVMS_PAT_CODE");
        DBMapStatic.put(txt_patInitials, table_static + "PVMS_PAT_INITIALS");
        DBMapStatic.put(cbx_regLead, table_static + "PVMS_ID_REG_LEAD");
        DBMapStatic.put(int_maxRep, table_static + "PVMS_MAX_REP");
        
        DBMapStatic.put(pdfType, table_static + "PVMS_PDF_TYPE");
        DBMapStatic.put(pdfFreq, table_static + "PVMS_PDF_FREQ");
    ...
    private void getStatic() {
        ...
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Einzeln(table_static);
        ...
        db.DB_Data_Get.DB_Filter.DB_WHERE_Allgemein(table_static, "PVMS_VH_ID", "=", wid, "");

        Container con = db.DB_Data_Get.DB_SEND_AND_GET_Container();
        Form_Bind fb = new Form_Bind();

        if (con.size() > 0) {
            staticId = Integer.toString((Integer) con.getItem(1).getItemProperty("PVMS_ID").getValue());
...
//        db.DB_Data_Get.DB_DEBUG_SQL_STRING();
    ...
    private void fillDynamicList() {
        ...
        DBClass db = new DBClass();
        db.DB_Data_Get.DB_Spalten.Standard.DB_set_Spalten_All();
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Einzeln(table_dynamic);
        ...
        db.DB_Data_Get.DB_Filter.DB_WHERE_Allgemein(table_dynamic, "PVMD_VH_ID", "=", wid, "");
        ...
        for (Object id : scon.getItemIds()) {
            scon.getItem(id)
                    ...
                    .setValue(
                            converter.convertToPresentation(scon.getItem(id).getItemProperty("PVMD_DELEGATION").getValue(), String.class,
                                    UI.getCurrent().getLocale()));
            sconId = id;
        }
        scon.addItem("new").getItemProperty("display").setValue(i18n.get(caption.Neu));
        list_dynamic.setContainerDataSource(scon);
        list_dynamic.setLocale(new Locale("de"));
    ...
    private RruleItem dbGetRecurrenceRule(Integer delegationId) {
        ...
        String sql = "select * from cust_protokoll_rrules"
            + "\n " + "where RRL_ID = (select PVMD_RRULE_ID from cust_protokoll_vpriv_meta_dynamic where PVMD_ID = " + delegationId + ")";
        q.setSqlString(sql);
    ...
    private Integer saveDynamic() {
        CMS_Config_Std std = CMS_Config_Std.getInstance();
        DBClass db = new DBClass();
        db.DB_Data_Insert.DB_Tabellen.DB_set_Tabelle_Einzeln(table_dynamic);
        Form_Bind fb = new Form_Bind();
        fb.Insert(db, DBMapDynamic);

        db.DB_Data_Insert.DB_Daten.DB_Data(table_dynamic, "PVMD_VH_ID", getMainId());
...
//        db.DB_Data_Insert.DB_DEBUG_SQL_STRING();
    ...
    private void updateDynamic() {
        ...
        DBClass db = new DBClass();
        db.DB_Data_Update.DB_Tabellen.DB_set_Tabelle_Einzeln("cust_protokoll_vpriv_meta_dynamic");
        db.DB_Data_Update.DB_Daten.DB_Data("cust_protokoll_vpriv_meta_dynamic", "PVMD_HOMECARE", cb_homecare.getValue() ? "1" : "0");

        db.DB_Data_Update.DB_Filter.DB_WHERE_Allgemein("cust_protokoll_vpriv_meta_dynamic", "PVMD_ID", "=", metaDynamicId.toString(), "");
...
//            dbc.DB_Data_Insert.DB_Daten.DB_Data(table_dynamic, "PVMD_VH_ID", getMainId());
...
//    }
    ...
    private Integer saveStatic() {
        CMS_Config_Std std = CMS_Config_Std.getInstance();
        DBClass db = new DBClass();
        db.DB_Data_Insert.DB_Tabellen.DB_set_Tabelle_Einzeln(table_static);

        Form_Bind fb = new Form_Bind();
        fb.Insert(db, DBMapStatic);

        db.DB_Data_Insert.DB_Daten.DB_Data(table_static, "PVMS_VH_ID", getMainId());
    ...
    private void updateStatic() {
        CMS_Config_Std std = CMS_Config_Std.getInstance();
        DBClass db = new DBClass();
        db.DB_Data_Update.DB_Tabellen.DB_set_Tabelle_Einzeln(table_static);

        Form_Bind fb = new Form_Bind();
        fb.Update(db, DBMapStatic);
        
        db.DB_Data_Update.DB_Filter.DB_WHERE_Allgemein(table_static, "PVMS_ID", "=", staticId, "");
    ...
    public void setRecurrenceSettings(String Form_ID){
        ...
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Verknüpfung("cust_verk_haupt", "INNER JOIN", table_static, "VH_ID", "PVMS_VH_ID");
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Verknüpfung("cust_verk_haupt", "INNER JOIN", table_dynamic, "VH_ID", "PVMD_VH_ID");
        db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Verknüpfung(table_dynamic, "INNER JOIN", "cust_protokoll_rrules", "PVMD_RRULE_ID", "RRL_ID");
        
        db.DB_Data_Get.DB_Ordnen.DB_Ordnen("PVMD_ACTIVE", "DESC");
        db.DB_Data_Get.DB_Ordnen.DB_Ordnen("PVMD_CREATED", "DESC");
        ...
        if(settings != null){
            ...
            RecurrenceRepetitions = (Integer) settings.getItemProperty("PVMS_MAX_REP").getValue();
            RecurrenceMetaId = (Integer) settings.getItemProperty("PVMD_ID").getValue();
        }
        ...
        dbb.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Verknüpfung("cust_protokoll_vpriv_daten", "INNER JOIN", table_dynamic, "PVD_DELEGATION_ID", "PVMD_ID");
        
        dbb.DB_Data_Get.DB_Ordnen.DB_Ordnen("PVD_ORIGINAL_DATE", "DESC");
        dbb.DB_Data_Get.DB_Limit.DB_LIMIT("1");
        
        dbb.DB_Data_Get.DB_Filter.DB_WHERE_Allgemein(table_dynamic, "PVMD_VH_ID", "=", Form_ID, "");
        
//        dbb.debugNextQuery(true);
        
        Item item = dbb.DB_Data_Get.DB_SEND_AND_GET_FIRST_ITEM();
        
        RecurrenceLatestDate = (item == null) ? null : (Date) item.getItemProperty("PVD_ORIGINAL_DATE").getValue();
