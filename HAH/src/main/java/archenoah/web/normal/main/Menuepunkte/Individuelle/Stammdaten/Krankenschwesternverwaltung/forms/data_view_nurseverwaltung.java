...
//    private String LNG_FRM_ID = "10";
    ...
    private Container Get_Data_Container() {
        try {
            ...
            db.DB_Data_Get.DB_Tabellen.DB_set_Tabelle_Verknüpfung_Graph("left join", "(SELECT KSF_ID_NURSE, GROUP_CONCAT(IF_NAME_ABRV SEPARATOR ', ') AS fcount"
                                                                 + "\n " + "FROM cust_krankenschwester_stammdaten_firmen"
                                                                 + "\n " + "left join cust_internal_firms on IF_KEY = KSF_KEY_FIRM"
                                                                 + "\n " + "where KSF_DATE_FROM <= NOW() and (KSF_DATE_UNTIL >= NOW() or KSF_DATE_UNTIL IS NULL)"
